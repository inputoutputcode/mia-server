...
public class SpectatorApplication {
	...
	private void initialize() {
		...
		scoresTableHeader = new JLabel("Waiting for the first score...");
		...
		lblHistorie = new JLabel("Score per minute");
		...
		roundHeader = new JLabel("Waiting for the first round...");
		roundHeader.setFont(new Font("Arial", Font.BOLD, 18));
		leftColumn.add(roundHeader, "cell 0 0 2 1");

		lblNchsteRunde = new JLabel("next round:");
	...
	public void updateRoundData(final int roundNumber, final String message) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				roundHeader.setText("Round run: " + roundNumber);
	...
	public void showScores(final int roundNumber, final Scores scores) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				scoresTableHeader.setText("Score for round "
						+ roundNumber);
